...
userRouter.get("/", async (req, res) => {
	try {
		...
		const users = await getMany({ db, query, params: [] });
...
userRouter.get("/:id", async (req, res) => {
	...
	const user = await getOne({ db, query, params: [userId] });
	if (user === null) {
...
userRouter.post("/", validateUser, async (req, res) => {
	...
	const query = "INSERT INTO users(fullname, age, gender) VALUES (?, ?, ?)";
...
userRouter.delete("/:id", async (req, res) => {
	...
	const user = await deleteOne({ db, query, params: [userId] });
